    ...
    return (
      <div>
        ...
        <table style={{ margin: 20 }} border="1" rules="all">
          ...
          <tbody>
            <tr>
              <TableData>{journalsCredit['売上高'].type}</TableData>
              <TableData>{journalsCredit['売上高'].account}</TableData>
              <TableData>
                {Number(journalsCredit['売上高'].total).toLocaleString()}
              </TableData>
            </tr>
            <tr>
              <TableData>{journalsDebit['仕入高'].type}</TableData>
              <TableData>{journalsDebit['仕入高'].account}</TableData>
              <TableData>
                {Number(journalsDebit['仕入高'].total).toLocaleString()}
              </TableData>
            </tr>
            <tr style={{ background: '#d3d3d3' }}>
              <TableData>売上総利益</TableData>
              <TableData />
              <TableData>{Number(GrossProfit).toLocaleString()}</TableData>
            </tr>
              ...
              return (
                journals.type == '販管費' && (
                  <tr key={Index}>
                    ...
                    <TableData>{journals.account}</TableData>
                    <TableData>
                      {Number(journals.total).toLocaleString()}
                    </TableData>
                  </tr>
                )
              )
            ...
            <tr style={{ background: '#d3d3d3' }}>
              ...
              <TableData />
              <TableData>{Number(salesProfit).toLocaleString()}</TableData>
            </tr>
            ...
            <tr style={{ background: '#d3d3d3' }}>
              ...
              <TableData />
              <TableData>{Number(currentProfit).toLocaleString()}</TableData>
            </tr>
            {originJournalsCredit.map((journals, Index) => {
              return (
                journals.type == '特別利益' && (
            ...
            <tr style={{ background: '#d3d3d3' }}>
              ...
              <TableData />
              <TableData>
                {Number(currentIncomeBeforeTax).toLocaleString()}
              </TableData>
            </tr>
            {originJournalsDebit.map((journals, Index) => {
              return (
            ...
            <tr style={{ background: '#d3d3d3' }}>
              ...
              <TableData />
              <TableData>{Number(netIncome).toLocaleString()}</TableData>
            </tr>
          </tbody>
        </table>
      </div>
